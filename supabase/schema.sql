...
-- SQL editor and run it once, on an empty project.
--
-- Wrapped in a transaction: Postgres applies DDL atomically, so if any
-- statement fails the whole script rolls back and the database is left
-- untouched rather than half-created. Fix the error and run it again.
