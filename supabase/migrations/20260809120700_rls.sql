...
create policy "class readers read the roster"
...
create policy "class readers read student contacts"
  on public.student_contacts for select to authenticated
  using (
    public.has_access()
    and exists (
      select 1 from public.students s
      where s.id = student_id and public.can_read_class(s.class_id)
    )
  );
...
create policy "teachers manage student contacts"
  ...
  using (
    ...
    and exists (
      select 1 from public.students s
      where s.id = student_id and public.owns_class(s.class_id)
    )
  )
  with check (
    public.has_access()
