-- Warq - fix save_attendance
--
-- Run this AFTER the other patches. Safe to run more than once.
--
-- The function declared a variable called session_id, which Postgres cannot
-- tell apart from attendance_records.session_id inside the INSERT ... SELECT
-- that writes the marks. It creates cleanly and then fails at run time with
-- 42702 "column reference session_id is ambiguous", so taking a register was
-- impossible. The variable is now v_session_id.
